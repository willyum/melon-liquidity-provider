...
router.get('/', function(req, res, next) {
  ...
  request('https://api.kraken.com/0/public/Ticker?pair=MLNETH', function (error, response, body) {
    if (!error && response.statusCode == 200) {
        ...
        var askData = [sellMLN, mlnAdd, buyETH, ethAdd];
        var bidData = [sellETH, ethAdd, buyMLN, mlnAdd];

        // var askId = contract.make(askData);
        // liveAskIDs.push(askId);
        // var bidId = contract.make(bidData);
        // liveBidIDs.push(bidId);
        
        console.log("Hello" + sellMLN);

        console.log(liveAskIDs);
        console.log(liveBidIDs);
        res.render('index', { liveAsk: ask, liveBid: bid, OrdersCancelled: [] });
    }
  });
});
